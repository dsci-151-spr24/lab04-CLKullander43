...
**There are `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents one Denny's location. The variables are `r names(dennys)`.**

2.  What are the dimensions of the La Quinta's dataset?
    What does each row in the dataset represent?
    What are the variables?

**There are `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents one La Quinta location. The variables are `r names(laquinta)`.**
...
**La Quinta locations are mostly in the US. However there are also locations in Canada, Mexico, China, New Zealand, Turkey, Arab, Chile, Colombia, and Ecuador. Denny's only has locations in the US.**
...
**take the dataset and use select to deselect rows when the country is US.**
...
**The data does not show any Denny's locations in other countries**
...
laquinta <- laquinta %>%
  mutate(country = case_when(
    ...
    state == "ANT"           ~ "Colombia",
    state %in% c("NL", "AG", "QR", "CH", "VE", "PU")   ~ "Mexico",
    state == "FM"            ~ "Honduras"
  ))
```
...
**Delaware, DC, and Vermont have the fewest Denny's locations, while California has the most.**
